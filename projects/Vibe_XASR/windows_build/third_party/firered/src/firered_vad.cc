...
#ifdef _WIN32
#include <windows.h>
// Windows port: ORT takes wchar_t* model paths (ORTCHAR_T = wchar_t). Convert from the system
// ANSI codepage — the C# side marshals the dir as CharSet.Ansi and mingw's std::ifstream also
// uses CP_ACP for char* paths, so a CJK install path stays consistent across both reads.
static std::wstring frv_widen(const std::string& s) {
...
#ifdef _WIN32
    v->sess.reset(new Ort::Session(*v->env, frv_widen(model).c_str(), *v->sopts));
#else
