...
import CSherpa     // SwiftPM: expose the sherpa-onnx C API (no bridging header)
...
class SherpaOnnxRecognizer {
  ...
  /// Start a brand-new decoding stream with pristine encoder/decoder state,
  /// matching Python's per-utterance `recognizer.create_stream()`. A plain
  /// Reset() can leave deferred-punctuation / decoder state that leaks the
  /// previous sentence's trailing punctuation into the next sentence.
  func newStream() {
    lock.lock()
    SherpaOnnxDestroyOnlineStream(stream)
...
class SourceSeparator {
  ...
  func process(buffer: AudioData) -> [AudioData]? {
    ...
    return buffer.withUnsafeBufferPointer { flatBuf -> [AudioData]? in
